...
export default async function HomePage() {
  ...
  // Fallback metric display values if DB isn't seeded yet
      ...
      <HeroSection>
        <div className="max-w-7xl mx-auto px-6 pt-32 pb-16">
          ...
          <Reveal direction="up" delay={0.1}>
            <h1 className="text-5xl md:text-7xl font-extrabold leading-[1.05] tracking-tight mb-6 max-w-4xl">
              Intelligent Solutions for a{' '}
              <span className="text-gradient">Sustainable Industry</span>
            </h1>
          </Reveal>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {displayMetrics.slice(0, 4).map((m: { labelEn: string; value: number; unit: string }, i: number) => (
                <div key={i} className="text-center">
                  <div className="text-4xl md:text-5xl font-extrabold text-gradient tabular-nums">
                    <AnimatedCounter
                      value={m.value}
                      suffix={m.unit.includes('%') ? '%' : '+'}
                    />
                  </div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6">
        <Reveal direction="up">
          <div className="mb-12">
            ...
            <h2 className="text-3xl md:text-4xl font-bold mt-2">Six Pillars of Innovation</h2>
          ...
          {domainCards.map((domain, i) => {
            ...
            return (
              <Reveal key={domain.slug} direction="up" delay={i * 0.07}>
                <Link href={`/technologies?domain=${domain.slug}`} className="block group">
                  <div className="glass rounded-xl p-6 h-full border border-[--border] hover:border-[--accent] transition-colors duration-300 cursor-pointer">
        ...
        <section className="py-24 bg-[--surface] border-y border-[--border]">
          <div className="max-w-7xl mx-auto px-6">
            <Reveal direction="up">
              <div className="flex items-end justify-between mb-12">
                <div>
                  ...
                  <h2 className="text-3xl md:text-4xl font-bold mt-2">Key Technologies</h2>
                </div>
            ...
            <div className="space-y-3">
              {technologies.slice(0, 4).map((tech: { id: string; nameEn: string; domain: { nameEn: string }; descEn: string; energySavingsPct: number | null; carbonReductionPct: number | null; tags: string[] }, i: number) => (
